...
void loop() {
  ...
  delay(lightTime);
  for (int i = 0; i < NUM_LEDS; i++)
    Red[i] = constrain(map(analogRead(A0),blackR[i] , whiteR[i], 0, 255), 0, 255);

  ledG();
  delay(lightTime);
  for (int i = 0; i < NUM_LEDS; i++)
    Green[i] = constrain(map(analogRead(A0),blackG[i] , whiteG[i], 0, 255), 0, 255);

  ledB();
  delay(lightTime);
  for (int i = 0; i < NUM_LEDS; i++)
    Blue[i] = constrain(map(analogRead(A0),blackB[i] , whiteB[i], 0, 255), 0, 255);
